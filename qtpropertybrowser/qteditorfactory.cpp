...
// ------------ QtYFileBoxFactoryPrivate
class QtYFileBoxFactoryPrivate : public EditorFactoryPrivate<QtFilePathBoxWidget>
{
    QtYFileBoxFactory *q_ptr;
    Q_DECLARE_PUBLIC(QtYFileBoxFactory)

public:
    void slotPropertyChanged(QtProperty *property, const QString &value);
    void slotSetValue(const QString &value);
    void slotReadOnlyChanged(QtProperty *property, const bool &val);
    void slotPathTypeChanged(QtProperty *property, bool type);
};

void QtYFileBoxFactoryPrivate::slotPropertyChanged(QtProperty *property, const QString &value)
{
    const PropertyToEditorListMap::iterator it = m_createdEditors.find(property);
    if (it == m_createdEditors.end())
        return;
    QListIterator<QtFilePathBoxWidget *> itEditor(it.value());

    while (itEditor.hasNext())
        itEditor.next()->setValue(value);
}

void QtYFileBoxFactoryPrivate::slotSetValue(const QString &value)
{
    ...
    for (EditorToPropertyMap::ConstIterator itEditor = m_editorToProperty.constBegin(); itEditor != ecend; ++itEditor)
        if (itEditor.key() == object) {
            QtProperty *property = itEditor.value();
            QtYFileBoxPropertyManager *manager = q_ptr->propertyManager(property);
            if (!manager)
                return;
...
void QtYFileBoxFactoryPrivate::slotReadOnlyChanged(QtProperty *property, const bool &val)
{
    ...
    QListIterator<QtFilePathBoxWidget *> itYFile(m_createdEditors[property]);
    while (itYFile.hasNext()) {
        QtFilePathBoxWidget *editor = itYFile.next();
...
void QtYFileBoxFactoryPrivate::slotPathTypeChanged(QtProperty *property, bool type)
{
    if (!m_createdEditors.contains(property))
        return;
    ...
    while (itYFile.hasNext()) {
        QtFilePathBoxWidget *editor = itYFile.next();
        editor->blockSignals(true);
        editor->setIsFilePathType(type);
        editor->blockSignals(false);
    }
}

/*!
    \class QtYFileBoxFactory

    \brief The QtYFileBoxFactory class provides YFileBox widgets for
    properties created by QtQStringPropertyManager objects.
...
*/
QtYFileBoxFactory::QtYFileBoxFactory(QObject *parent)
    : QtAbstractEditorFactory<QtYFileBoxPropertyManager>(parent),
    d_ptr(new QtYFileBoxFactoryPrivate())
{
    d_ptr->q_ptr = this;
}
/*!
...
*/
QtYFileBoxFactory::~QtYFileBoxFactory()
{
...
*/
void QtYFileBoxFactory::connectPropertyManager(QtYFileBoxPropertyManager *manager)
{
...
QWidget *QtYFileBoxFactory::createEditor(QtYFileBoxPropertyManager *manager,
        QtProperty *property, QWidget *parent)
{

    QtFilePathBoxWidget *editor = d_ptr->createEditor(property, parent);
    editor->setReadOnly(manager->isReadOnly(property));
    editor->setValue(manager->value(property));
    editor->setIsFilePathType(manager->isFilePathType(property));

    connect(editor, SIGNAL(valueChanged(const QString &)),
            this, SLOT(slotSetValue(const QString &)));

    // connect(editor, SIGNAL(pathTypeChanged(QtProperty *, const bool &)),
    //         this, SLOT(slotPathTypeChanged(QtProperty *, const bool &)));
...
/*!
    ...
    Reimplemented from the QtAbstractEditorFactory class.
*/
void QtYFileBoxFactory::disconnectPropertyManager(QtYFileBoxPropertyManager *manager)
{
    disconnect(manager, SIGNAL(valueChanged(QtProperty *, const QString &)),
               this, SLOT(slotPropertyChanged(QtProperty *, const QString &)));

    disconnect(manager, SIGNAL(pathTypeChanged(QtProperty *, const bool &)),
            this, SLOT(slotPathTypeChanged(QtProperty *, const bool &)));
