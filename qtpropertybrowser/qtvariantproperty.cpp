...
Q_DECLARE_METATYPE(QtYFileBoxPropertyType)
...
int QtVariantPropertyManager::fileBoxTypeId()
{
    return qMetaTypeId<QtYFileBoxPropertyType>();
}
...
public:
    ...
    void slotReadOnlyChanged(QtProperty *property, const bool &readOnly);
    void slotTextVisibleChanged(QtProperty *property, bool textVisible);
    void slotSetFilePathTypeChanged(QtProperty *property, const bool &isFilePath);
    ...
    const QString m_setFilePathAttribute;
};
...
QtVariantPropertyManagerPrivate::QtVariantPropertyManagerPrivate() :
    ...
    m_setFilePathAttribute(QLatin1String("setFilePathType"))
{
}
...
void QtVariantPropertyManagerPrivate::slotReadOnlyChanged(QtProperty *property, const bool &readOnly)
{
    if (QtVariantProperty *varProp = m_internalToProperty.value(property, 0))
...
void QtVariantPropertyManagerPrivate::slotSetFilePathTypeChanged(QtProperty *property, const bool &isFilePath)
{
    if (QtVariantProperty *varProp = m_internalToProperty.value(property, 0))
        emit q_ptr->attributeChanged(varProp, m_setFilePathAttribute, QVariant(isFilePath));
}
    ...
    : QtAbstractPropertyManager(parent)
{
    ...
    // YFileBoxPropertyManager
    int yfileBoxId = fileBoxTypeId();
    QtYFileBoxPropertyManager *yfileBoxPropertyManager = new QtYFileBoxPropertyManager(this);
    d_ptr->m_typeToPropertyManager[yfileBoxId] = yfileBoxPropertyManager;
    d_ptr->m_typeToValueType[yfileBoxId] = QVariant::String;
    d_ptr->m_typeToAttributeToAttributeType[yfileBoxId][d_ptr->m_readOnlyAttribute] =
        QVariant::Bool;
    d_ptr->m_typeToAttributeToAttributeType[yfileBoxId][d_ptr->m_setFilePathAttribute] =
        QVariant::Bool;
    connect(yfileBoxPropertyManager, SIGNAL(valueChanged(QtProperty *, const QString&)),
            this, SLOT(slotValueChanged(QtProperty *, const QString&)));

    connect(yfileBoxPropertyManager, SIGNAL(readOnlyChanged(QtProperty *, const bool &)),
            this, SLOT(slotReadOnlyChanged(QtProperty *, const bool &)));

    connect(yfileBoxPropertyManager, SIGNAL(pathTypeChanged(QtProperty *, const bool &)),
            this, SLOT(slotSetFilePathTypeChanged(QtProperty *, const bool &)));

    connect(yfileBoxPropertyManager, SIGNAL(propertyInserted(QtProperty *, QtProperty *, QtProperty *)),
            this, SLOT(slotPropertyInserted(QtProperty *, QtProperty *, QtProperty *)));
    connect(yfileBoxPropertyManager, SIGNAL(propertyRemoved(QtProperty *, QtProperty *)),
            this, SLOT(slotPropertyRemoved(QtProperty *, QtProperty *)));
    // FlagPropertyManager
...
QVariant QtVariantPropertyManager::value(const QtProperty *property) const
{
    ...
    } else if (QtYFileBoxPropertyManager *yfileBoxManager = qobject_cast<QtYFileBoxPropertyManager *>(manager)) {
        return yfileBoxManager->value(internProp);
    }
    return QVariant();
}
...
QVariant QtVariantPropertyManager::attributeValue(const QtProperty *property, const QString &attribute) const
{
    ...
    if (QtIntPropertyManager *intManager = qobject_cast<QtIntPropertyManager *>(manager)) {
        ...
        return QVariant();
    } else if (QtYFileBoxPropertyManager *yfileManager = qobject_cast<QtYFileBoxPropertyManager *>(manager)) {
        if (attribute == d_ptr->m_readOnlyAttribute)
            return yfileManager->isReadOnly(internProp);
        if (attribute == d_ptr->m_setFilePathAttribute)
            return yfileManager->isFilePathType(internProp);
        return QVariant();
    }
    return QVariant();
}
...
void QtVariantPropertyManager::setValue(QtProperty *property, const QVariant &val)
{
    ...
    if (QtIntPropertyManager *intManager = qobject_cast<QtIntPropertyManager *>(manager)) {
        ...
        yfileManager->setValue(internProp, qvariant_cast<QString>(val));
        return;
    }
}
        ...
        const QString &attribute, const QVariant &value)
{
    ...
    if (QtIntPropertyManager *intManager = qobject_cast<QtIntPropertyManager *>(manager)) {
        ...
        if (attribute == d_ptr->m_readOnlyAttribute)
            yfileBoxManager->setReadOnly(internProp, (EchoMode)qvariant_cast<bool>(value));
        if (attribute == d_ptr->m_setFilePathAttribute)
            yfileBoxManager->setPathType(internProp, (EchoMode)qvariant_cast<bool>(value));
        return;
    }
}
...
public:
    ...
    QtYFileBoxFactory          *m_yFileBoxEditorFactory;
    ...
    : QtAbstractEditorFactory<QtVariantPropertyManager>(parent)
{
    ...
    d_ptr->m_yFileBoxEditorFactory = new QtYFileBoxFactory(this);
    const int yfileBoxId = QtVariantPropertyManager::fileBoxTypeId();
    d_ptr->m_factoryToType[d_ptr->m_yFileBoxEditorFactory] = yfileBoxId;
    d_ptr->m_typeToFactory[yfileBoxId] = d_ptr->m_yFileBoxEditorFactory;
}
...
void QtVariantEditorFactory::connectPropertyManager(QtVariantPropertyManager *manager)
{
    ...
    QList<QtYFileBoxPropertyManager *> yFileBoxPropertyManagers = manager->findChildren<QtYFileBoxPropertyManager *>();
    QListIterator<QtYFileBoxPropertyManager *> ityFileBox(yFileBoxPropertyManagers);
    while (ityFileBox.hasNext())
        d_ptr->m_yFileBoxEditorFactory->addPropertyManager(ityFileBox.next());
...
void QtVariantEditorFactory::disconnectPropertyManager(QtVariantPropertyManager *manager)
{
    ...
    QList<QtYFileBoxPropertyManager *> yfilePropertyManagers = manager->findChildren<QtYFileBoxPropertyManager *>();
    QListIterator<QtYFileBoxPropertyManager *> itYFile(yfilePropertyManagers);
    while (itString.hasNext())
        d_ptr->m_yFileBoxEditorFactory->removePropertyManager(itYFile.next());
}
