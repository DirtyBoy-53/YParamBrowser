...
class QtYFileBoxFactoryPrivate;

class QT_QTPROPERTYBROWSER_EXPORT QtYFileBoxFactory : public QtAbstractEditorFactory<QtYFileBoxPropertyManager>
{
    Q_OBJECT
public:
    QtYFileBoxFactory(QObject *parent = 0);
    ~QtYFileBoxFactory();
protected:
    void connectPropertyManager(QtYFileBoxPropertyManager *manager);
    QWidget *createEditor(QtYFileBoxPropertyManager *manager, QtProperty *property,
                          QWidget *parent);
    void disconnectPropertyManager(QtYFileBoxPropertyManager *manager);
private:
    QtYFileBoxFactoryPrivate *d_ptr;
    Q_DECLARE_PRIVATE(QtYFileBoxFactory)
    Q_DISABLE_COPY(QtYFileBoxFactory)
    Q_PRIVATE_SLOT(d_func(), void slotPropertyChanged(QtProperty *, const QString &))
    Q_PRIVATE_SLOT(d_func(), void slotReadOnlyChanged(QtProperty *, bool))
    Q_PRIVATE_SLOT(d_func(), void slotPathTypeChanged(QtProperty *, bool))
