...
// QtYFileBoxPropertyManager
class QtYFileBoxPropertyManagerPrivate
{
    QtYFileBoxPropertyManager *q_ptr;
    Q_DECLARE_PUBLIC(QtYFileBoxPropertyManager)
public:
    struct Data
    {
        Data() : readOnly(true),pathType("file") { }
        QString val;
        bool readOnly;
        bool pathType;
    };
...
QtYFileBoxPropertyManager::QtYFileBoxPropertyManager(QObject *parent)
    : QtAbstractPropertyManager(parent)
{
    d_ptr = new QtYFileBoxPropertyManagerPrivate;
    d_ptr->q_ptr = this;
}
/*!
    Destroys this manager, and all the properties it has created.
*/
QtYFileBoxPropertyManager::~QtYFileBoxPropertyManager()
{
    clear();
    delete d_ptr;
}
...
/*!
    ...
    \sa setValue()
*/
QString QtYFileBoxPropertyManager::value(const QtProperty *property) const
{
    return getValue<QString>(d_ptr->m_values, property, "");
}
...
/*!
    ...
    \sa QtStringPropertyManager::setReadOnly
*/
bool QtYFileBoxPropertyManager::isReadOnly(const QtProperty *property) const
{
    return getData<bool>(d_ptr->m_values, &QtYFileBoxPropertyManagerPrivate::Data::readOnly, property, false);
}

bool QtYFileBoxPropertyManager::isFilePathType(const QtProperty *property) const
{
    return getData<bool>(d_ptr->m_values, &QtYFileBoxPropertyManagerPrivate::Data::pathType, property, true);
}
/*!
    \reimp
*/
QString QtYFileBoxPropertyManager::valueText(const QtProperty *property) const
{
    const QtYFileBoxPropertyManagerPrivate::PropertyValueMap::const_iterator it = d_ptr->m_values.constFind(property);
    if (it == d_ptr->m_values.constEnd())
        return QString();
...
void QtYFileBoxPropertyManager::setValue(QtProperty *property, const QString &val)
{
    const QtYFileBoxPropertyManagerPrivate::PropertyValueMap::iterator it = d_ptr->m_values.find(property);
    if (it == d_ptr->m_values.end())
        return;

    QtYFileBoxPropertyManagerPrivate::Data data = it.value();
...
/*!
    ...
    \sa QtStringPropertyManager::setReadOnly
*/
void QtYFileBoxPropertyManager::setReadOnly(QtProperty *property, bool readOnly)
{
    const QtYFileBoxPropertyManagerPrivate::PropertyValueMap::iterator it = d_ptr->m_values.find(property);
    if (it == d_ptr->m_values.end())
        return;
...
void QtYFileBoxPropertyManager::setPathType(QtProperty *property, const bool &val)
{
    const QtYFileBoxPropertyManagerPrivate::PropertyValueMap::iterator it = d_ptr->m_values.find(property);
    if (it == d_ptr->m_values.end())
        return;
    ...
    emit propertyChanged(property);
    emit pathTypeChanged(property, data.pathType);
}

/*!
    \reimp
*/
void QtYFileBoxPropertyManager::initializeProperty(QtProperty *property)
{
    d_ptr->m_values[property] = QtYFileBoxPropertyManagerPrivate::Data();
}

/*!
    \reimp
*/
void QtYFileBoxPropertyManager::uninitializeProperty(QtProperty *property)
{
    d_ptr->m_values.remove(property);
}
